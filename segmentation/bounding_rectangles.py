...
from connected_components import _bounded_coords
...
def draw_rects(img: ArrayLike, rects: list[Rectangle],
    ...
    @param color: color of the rectangles, a tuple of 3 integers (plus an additional float) between 0 and 255,
    ...
    if not isinstance(img, np.ndarray):
        try:
            ...
            raise ValueError("Image must be convertible to a numpy array with uint8 values")
    ...
    if color is not None and (sum([i is not None and (i < 0 or i > 255) for i in color[:3]]) > 0 or
                              (len(color) == 4 and color[3] is not None and not 0 <= color[3] <= 1)):
...
def intersection_over_union(rect1: Rectangle, rect2: Rectangle):
    ...
    intersection = (min(br1[0], br2[0]) - max(tl1[0], tl2[0])) * (min(br1[1], br2[1]) - max(tl1[1], tl2[1]))
