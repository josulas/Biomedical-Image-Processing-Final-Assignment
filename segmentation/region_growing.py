...
class ConnectionCriteria:
    ...
    """
    def __init__(self, connectivity: int = NeighborhoodTypes.N4, mode: int = RegionGrowingModes.FIXED,
                 params: tuple[int, int] | int = (0, 255)):
        if connectivity not in {NeighborhoodTypes.N4, NeighborhoodTypes.N8}:
            raise ValueError(f"Connectivity must be {NeighborhoodTypes.N4} for 'N4' or {NeighborhoodTypes.N8} for 'N8',\
             got{connectivity} instead.")
        if mode not in list(RegionGrowingModes()):
            raise ValueError(f"Mode must be {RegionGrowingModes.FIXED} for 'fixed' or {RegionGrowingModes.ADAPTIVE} for\
             'adaptive', got{mode} instead.")
        if mode == RegionGrowingModes.FIXED and (not isinstance(params, tuple) or not 0 <= params[0] <= params[1]):
            raise ValueError(f"Params must be a tuple of two positive integers, the second greater than the first,\
             got {params} instead.")
        if mode == RegionGrowingModes.ADAPTIVE and (not isinstance(params, int) or params < 1):
            raise ValueError(f"Params must be a positive integer for the adaptive mode, got {params} instead.")
...
-> NDArray[np.integer]:
    """
    Takes a one-channel image, a seed value, a criterion, and returns a binary image with the same size as the input image.
    ...
    if type(params) != tuple: # Avoids linter errors
        params = (params, 0)
    neighbor_type = criterion.connectivity

    def _get_neighbors(coordinates_tuple: tuple[int, int], connectivity: int) -> set[tuple[int, int]]:
        neighbors_set = set()
    ...
    def _is_in_shape(coordinates_tuple: tuple[int, int], mode: int, params_tuple: tuple[int, int]) -> bool:
        y, x = coordinates_tuple
    ...
    if criterion.mode == RegionGrowingModes.FIXED:
        ...
        while queue:
            ...
            neighbors = _get_neighbors(coordinates, neighbor_type)
            for neighbor in neighbors:
                if result[neighbor] == 0 and _is_in_shape(neighbor, criterion.mode, (params[0], parent_val)):
                    result[neighbor] = 1
