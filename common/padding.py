...
numpy_real_type = type(numpy_real)
...
def zero_pad_img(img: ArrayLike, padding_width: int) -> NDArray[np.number]:
    ...
    img_padded = np.zeros((img.shape[0] + 2 * padding_width, img.shape[1] + 2 * padding_width), dtype=img.dtype)
    img_padded[padding_width:padding_width + img.shape[0], padding_width:padding_width + img.shape[1]] = img
    return img_padded
