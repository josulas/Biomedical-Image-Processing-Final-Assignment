"""
This module contains functions to compute various similarity indices and distance metrics
between tensors, such as binary images or other ND arrays.
"""
...
class SimilarityIndex:
    """
    Class to define similarity indices between two binary images.
    """
    DICE = 0
    JACCARD = 1
    def __iter__(self):
...
def _are_comparable(tensor1: ArrayLike,
                    tensor2: ArrayLike) -> tuple[NDArray[np.float64], NDArray[np.float64]]:
    """
    ...
    if not isinstance(tensor1, np.ndarray):
        tensor1 = np.array(tensor1).astype(np.float64)
    if not isinstance(tensor2, np.ndarray):
        tensor2 = np.array(tensor2).astype(np.float64)
    if not issubclass(tensor1.dtype.type, NumpyReal):
        raise ValueError("tensor1 must be a numpy array with real values")
    if not issubclass(tensor2.dtype.type, NumpyReal):
...
def l_norm(tensor1: ArrayLike, tensor2: ArrayLike, l: int) -> np.float64:
    ...
    """
    tensor1, tensor2 = _are_comparable(tensor1, tensor2)
    if l < 1:
...
def mse(tensor1: ArrayLike, tensor2: ArrayLike) -> np.float64:
    ...
    """
    tensor1, tensor2 = _are_comparable(tensor1, tensor2)
    return np.mean((tensor1 - tensor2) ** 2).astype(np.float64)
