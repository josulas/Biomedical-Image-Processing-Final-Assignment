"""
This file contains classes for JPEG sampling modes
"""
...
class JPEGSubSampler:
    """Implementation JPEG Sub-sampling"""
    ...
    def subsample(self, y_cr_cb_image: NDArray) -> tuple[NDArray, NDArray, NDArray]:
        """Implementation of subsampling according to chosen mode"""
        ...
        match self.mode:
            ...
            case (JPEGSubSampleModes.S4_2_2):
                cr_channel = resize(y_cr_cb_image[:, :, 1],
                                    (height, width // 2),
                                    preserve_range=True)
                cb_channel = resize(y_cr_cb_image[:, :, 2],
                                    (height, width // 2),
                                    preserve_range=True)
    ...
    def get_subsampling_factors(self):
        """Obtain sampling factors from chosen mode"""
        match self.mode:
            case (JPEGSubSampleModes.S4_4_4):
