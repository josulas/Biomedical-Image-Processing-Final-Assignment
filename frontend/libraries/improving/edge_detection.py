"""
This module contains functions to detect edges in pictures
"""
...
from libraries.improving.filtering import (conv2d,
                                           ...
                                           sobel_x,
                                           sobel_y)
from libraries.common.padding import extension_pad_img
...
def _validate_parameters(img: NDArray, mode: str, smooth: bool, sigma: Real | None) -> None:
    if mode not in ['same', 'valid']:
        raise ValueError("Mode must be 'same' or 'valid'")
    if not issubclass(img.dtype.type, NumpyReal):
        raise ValueError(f"Image values must be a numpy real numeric type, \
                         got {img.dtype.type} instead")
    if not img.ndim == 2:
...
def _smooth_image(img: NDArray, sigma: Real | None = None) -> NDArray:
    if sigma is None:
        sigma = 1
...
def canny(img: ArrayLike, t1: Real, t2: Real, mode: str = 'same',
    ...
    def horizontal(n8_neighborhood):  # -
        return n8_neighborhood[1, 1] >= n8_neighborhood[1, 0] and \
            n8_neighborhood[1, 1] >= n8_neighborhood[1, 2]
    def vertical(n8_neighborhood):    # |
        return n8_neighborhood[1, 1] >= n8_neighborhood[0, 1] and \
            n8_neighborhood[1, 1] >= n8_neighborhood[2, 1]
    def diag_ul_br(n8_neighborhood):  # \
        return n8_neighborhood[1, 1] >= n8_neighborhood[0, 0] and \
            n8_neighborhood[1, 1] >= n8_neighborhood[2, 2]
    def diag_ur_bl(n8_neighborhood):  # /
        return n8_neighborhood[1, 1] >= n8_neighborhood[0, 2] and \
            n8_neighborhood[1, 1] >= n8_neighborhood[2, 0]
    ...
    def connect_weak_edges(edges_matrix, gradient_magnitude, gradient_direction, k, l):
        ...
        try:
            if not edges_matrix[i_plus, j_plus] and \
                gradient_magnitude[i_plus, j_plus] > t1:
                edges_matrix[i_plus, j_plus] = 1
                connect_weak_edges(edges_matrix, gradient_magnitude, gradient_direction, i_plus, j_plus)
        except IndexError:
            pass
        try:
            if not edges_matrix[i_minus, j_minus] and \
                gradient_magnitude[i_minus, j_minus] > t1:
                edges_matrix[i_minus, j_minus] = 1
                connect_weak_edges(edges_matrix,
                                   gradient_magnitude,
                                   gradient_direction,
                                   i_minus,
                                   j_minus)
    ...
    gradient_angle = \
        ((1 * (gradient_y < 0) + \
          np.arctan2(gradient_y, gradient_x) / np.pi) * 4).astype(np.uint8)
