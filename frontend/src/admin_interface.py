...
class AdminInterface:
    ...
    def _show_create_user_form(self):
        ...
        with st.form("create_user_form"):
            ...
            is_active = st.checkbox("Active Account",
                                    value=True,
                    ...
                    if password != confirm_password:
                        ...
                        with st.spinner("Creating user..."):
                            ...
                            if result["success"]:
                                ...
                                st.error(f"❌ User creation failed: \
    ...
    def _show_edit_users_section(self):
        ...
        with col1:
            role_filter = st.selectbox("Filter by Role",
                                       options=["All", "patient", "neurologist", "admin"],
                                       index=0)
        with col2:
        ...
        for user in filtered_users:
            ...
            user_label = f"👤 {user['first_name']} {user['last_name']} ({user['role']}) \
            ...
            with st.expander(f"👤 {user['first_name']} {user['last_name']} ({user['role']}) \
                ...
                if user['id'] == current_admin_id:
                    st.warning("⚠️ You cannot edit your own account through user management. \
    ...
    def _show_edit_user_form(self, user: Dict[str, Any]):
        ...
        if user['id'] == current_user['id']:
            st.warning("⚠️ You cannot edit your own account through user management. \
                       Use Account Settings in the sidebar instead.")
            return
        ...
        with st.form(form_key):
            ...
            with col1:
                first_name = st.text_input("First Name",
                                           ...
                                           key=f"fn_{user['id']}")
                st.text_input("Email", value=user['email'], disabled=True, key=f"em_{user['id']}")
    ...
    def _show_create_assignment_form(self):
        ...
        with st.form("create_assignment_form"):
            ...
            with col1:
                patient_options = \
                    [f"{p['first_name']} {p['last_name']} ({p['email']})" for p in patients]
            ...
            with col2:
                neurologist_options = \
                    [f"Dr. {n['first_name']} {n['last_name']} ({n['email']})" \
                     for n in neurologists]
                selected_neurologist_idx = \
                    st.selectbox("Select Neurologist",
            ...
            if submit_assignment:
                with st.spinner("Creating assignment..."):
                    ...
                    if result["success"]:
                        st.success(f"✅ Patient {selected_patient['first_name']} \
                                   {selected_patient['last_name']} assigned to \
                                    Dr. {selected_neurologist['first_name']} \
                                        {selected_neurologist['last_name']}!")
                        st.rerun()
                    else:
    ...
    def _show_manage_assignments_section(self):
        ...
        with col1:
            status_filter = st.selectbox("Filter by Status",
                                         ...
                                         key="assign_filter")
        with col2:
            search_term = st.text_input("Search assignments",
                                        placeholder="Search by patient or neurologist name...",
                                        key="assign_search")
        # Filter assignments
        ...
        for assignment in filtered_assignments:
            ...
            with st.expander(f"{status_icon} {assignment['patient_name']} \
                             ↔ Dr. {assignment['neurologist_name']} \
                                (Assigned: {assignment['assigned_at'][:10]})"):
        ...
        if search_term:
            ...
            filtered = [a for a in filtered \
                        if search_lower in a['patient_name'].lower() \
                            or search_lower in a['neurologist_name'].lower()]
        return filtered
    ...
    def _show_edit_assignment_form(self, assignment: Dict[str, Any]):
        ...
        with st.form(form_key):
            ...
            neurologist_options = [f"Dr. {n['first_name']} {n['last_name']} ({n['email']})"\
                                   for n in neurologists]
            try:
                current_idx = next(i for i, n\
                                    in enumerate(neurologists) \
                                        if n['id'] == current_neurologist_id)
            ...
            is_active = st.checkbox("Active Assignment",
                                    value=assignment['is_active'],
                                    key=f"assign_active_{assignment['id']}")
            # Warning about neurologist change
            if new_neurologist['id'] != current_neurologist_id:
                st.warning("⚠️ **Warning:** Changing the neurologist will delete all diagnostic \
                           data (study results) associated with this assignment. \
                           Images will be preserved.")
            col_save, col_delete = st.columns(2)
            with col_save:
                submit_edit = st.form_submit_button("💾 Save Changes", use_container_width=True)
            with col_delete:
                submit_delete = st.form_submit_button("🗑️ Delete Assignment",
                                                      use_container_width=True,
                                                      type="secondary")
            if submit_edit:
