"""
This module contains functions to compute the iterative reconstruction
of tomography sinograms
"""
...
from libraries.reconstruction.common import cut_circle, cut_square
...
def kaczmarz_art(a: ArrayLike, b: ArrayLike, lambda_conv: float = 1,
    ...
    if not isinstance(a, np.ndarray):
        a = np.array(a)
    if not issubclass(a.dtype.type, NumpyReal) or not a.ndim == 2:
        raise ValueError("A must be a real-valued 2D array")
    n_row_a, n_col_a = a.shape
    if not isinstance(b, np.ndarray):
        b = np.array(b)
    if not issubclass(b.dtype.type, NumpyReal) or not b.ndim == 1:
