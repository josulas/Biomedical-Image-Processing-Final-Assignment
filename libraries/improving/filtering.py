...
def conv2d(img: ArrayLike, kernel: ArrayLike,
    ...
    if not issubclass(kernel.dtype.type, NumpyReal):
    ...
    if depth is not None and not issubclass(depth, NumpyRealType):
    ...
    img_filtered = np.zeros((working_img.shape[0] - reduction,
                             working_img.shape[1] - reduction),
                             dtype=depth)

    # Convolution. Note that for asymmetric convolutions,
    # the last row and column of the zero-padded image is never used
...
def median_filter(img: ArrayLike, mask_size: int = 3, mode: str = 'same') -> NDArray:
    """
    ...
    if not isinstance(img, np.ndarray):
        img = np.array(img)
    if img.ndim != 2 or not issubclass(img.dtype.type, NumpyReal):
        raise ValueError('Image must be a 2D array containing real values')
    if not isinstance(mask_size, int) or mask_size < 2:
        raise ValueError(f'Mask size must be an integer greater or \
                         equal than 2, got {mask_size} instead')
