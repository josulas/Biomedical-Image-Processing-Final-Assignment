"""
Mathematical morphology operations for binary images.
"""
...
def _validate_parameters(img: NDArray, structuring_element: ArrayLike, mode: str) -> None:
    """
    ...
    if not issubclass(structuring_element.dtype.type, np.integer) \
        or not structuring_element.ndim == 2:
    ...
    if not ((structuring_element == 0) | \
            (structuring_element == 1) | \
            np.isnan(structuring_element)).all():
...
def fit(patch: NDArray, structuring_element: NDArray) -> int:
    ...
    :return: 1 if all the non-zero elements of the structuring
    elements are matched with non-zero elements in the patch
    """
...
def hit(patch: NDArray, structuring_element: NDArray) -> int:
    ...
    :param patch: binary 2D NDArray
    :param structuring_element: binary 2D NDArray
    :return: 1 if at least one non-zero element of the 
    structuring elements is matched with any non-zero element
    in the patch
    """
    return 1 if np.sum(patch * structuring_element) else 0


def _iterate_over_image(img: ArrayLike,
                        structuring_element: ArrayLike,
                        mode: str,
                        operation: Callable) -> NDArray:
    """
    ...
    result = np.zeros((img_height - (el_height // 2) * 2,
                       img_width - (el_height // 2) * 2),
                       dtype=img.dtype)
    for i in range(result.shape[0]):
        for j in range(result.shape[1]):
            result[i, j] = operation(working_img[i:i + el_height, j:j + el_width],
                                     structuring_element)
    return result


def erosion(img: ArrayLike,
            structuring_element: ArrayLike,
            mode: str = 'same') -> NDArray[np.integer]:
    """
...
def closing(img: ArrayLike,
            structuring_element: ArrayLike,
            mode: str = 'same') -> NDArray[np.integer]:
    """
    Performs the closing operation over an Image: dilation followed by erosion
    @param img: ArrayLike containing integer values, only 1s and 0s
    @param structuring_element: ArrayLike containing integer values, 
                                only 1s, 0s and NaNs
    @param mode: Mode of operation, 'same' or 'valid'
    """
