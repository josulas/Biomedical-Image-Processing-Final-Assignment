"""
This modules implments the Kmeans algorithm for clustering numeric data.
"""
...
def kmeans(arr: ArrayLike,
           k: int = 2,
           best_labels: ArrayLike | None = None,
           criteria: KmeansTermCrit = KmeansTermCrit(KmeansTermOpt.BOTH,
                                                     10,
                                                     1e-6),
           attempts: int = 10,
           flags: int = KmeansFlags.RANDOM_CENTERS) \
    ...
    if best_labels is not None:
        if flags == KmeansFlags.RANDOM_CENTERS or flags == KmeansFlags.KMEANS_PP_CENTERS:
            raise ValueError("bestLabels must be None when flags \
                             is RANDOM_CENTERS or KMEANS_PP_CENTERS")
    ...
    def _kmeans_pp_init() -> NDArray:
        ...
        for center_index in range(1, k):
            for created_center_index in range(center_index):
                distances_pp_init[created_center_index] = \
                    np.sum((arr - centers_pp_init[created_center_index]) ** 2, axis = 1)
    ...
    def _k_means_step(centers_array):
        ...
        for j in range(k):
            centers_array[j] = \
                np.mean(arr[labels_array == j], axis=0) \
                if len(arr[labels_array == j]) \
    ...
    if flags == KmeansFlags.RANDOM_CENTERS or flags == KmeansFlags.CUSTOM_CENTERS_RANDOM:
        ...
        raise ValueError(f"flags must be RANDOM_CENTERS: {KmeansFlags.RANDOM_CENTERS}, "
                         ...
                         f"CUSTOM_CENTERS_PP: {KmeansFlags.CUSTOM_CENTERS_PP}, \
                            got {flags} instead.")

    if criteria.opt == KmeansTermOpt.EPS:
        termination = \
            lambda c, pc, ii: np.linalg.norm(c - pc) < criteria.epsilon
    elif criteria.opt == KmeansTermOpt.MAX_ITER:
        termination = \
            lambda c, pc, ii: ii >= criteria.max_iter
    elif criteria.opt == KmeansTermOpt.BOTH:
        termination = \
            lambda c, pc, ii: np.linalg.norm(c - pc) < criteria.epsilon or ii >= criteria.max_iter
    else:
        raise ValueError(f"criteria.opt must be EPS: {KmeansTermOpt.EPS}, \
        MAX_ITER: {KmeansTermOpt.MAX_ITER}, BOTH: {KmeansTermOpt.BOTH}, \
        got {criteria.opt} instead.")
