"""
Simple thresholding methods for image segmentation.
"""
...
from libraries.common.histogram import img2hist
...
def otsu(img: ArrayLike) -> np.integer:
    """
    Takes a one-channel image and returns the threshold value that 
...
class ThresholdType(int):
    """
    Enum-like class for thresholding types.
    """
    BINARY = 0
    BINARY_INV = 1
    TRUNC = 2
...
def apply_threshold(img: ArrayLike, threshold: int, t_type: ThresholdType,
    ...
    match t_type:
        case ThresholdType.BINARY:
            ...
            return np.where(_binary_t(img, threshold), max_v, min_v)
        case ThresholdType.BINARY_INV:
            ...
            return np.where(_binary_inv_t(img, threshold), max_v, min_v)
