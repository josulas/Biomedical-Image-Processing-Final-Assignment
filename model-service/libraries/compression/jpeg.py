...
from libraries.compression.jfif_definitions import (JFIFMarkers,
                                          ...
                                          JFIFHTClass,
                                          JPEGStandardTables)
from libraries.compression.jpeg_subsampling import JPEGSubSampleModes, JPEGSubSampler
from libraries.compression.jfif_block_building import (build_dqt_segment,
                                                       build_sof_segment,
                                                       build_dht_segment,
                                                       build_sos_segment)
from libraries.compression.jpeg_dct_encoding import (get_dct_encoded_channels,
                                                     get_channel_codebooks,
                                                     encode_channel_blocks,
                                                     get_encoded_scan)

class ImgType:
    """Contains ImageTypes Definitions"""
    GRAYSCALE = 0
    RGB = 1
    RGBA = 2
    def __iter__(self):
...
def encode_baseline(channels: list[NDArray[np.uint8]],
                    ...
                    dc_range: tuple[int, int] = (-2047, 2047),
                    ac_range: tuple[int, int] = (-1023, 1023)) -> bytearray:
    """
    Baseline encoding workflow
    """
    if dc_range != (-2047, 2047) or ac_range != (-1023, 1023):
        raise NotImplementedError("Cannot set custom ranges yet")
    # Apply DCT and RL Encoding
    dct_encoded_channels = \
        get_dct_encoded_channels(channels, quantization_matrices, quantization_matrices_ids)
    # Build Huffman's codebooks
    first_channel_codebook = \
        get_channel_codebooks(dct_encoded_channels[0].flatten())
    channels_codebooks = [first_channel_codebook]
    if len(dct_encoded_channels) > 1:
        remaining_channels_codebook = \
            get_channel_codebooks(np.concatenate([channel_blocks.flatten() \
                                                  for channel_blocks in dct_encoded_channels[1:]]))
        channels_codebooks.extend([remaining_channels_codebook \
                                   for _ in range(len(dct_encoded_channels) - 1)])
    codebook_indexes = [0]
    ...
    encoded_channels_mcus = [encode_channel_blocks(channel_blocks, *codebooks)
                             for channel_blocks, codebooks in \
                                zip(dct_encoded_channels, channels_codebooks)]
    ...
    for codebook_index, (dc_data_codebook, ac_data_codebook) in \
        zip(codebook_indexes, channels_codebooks):
    ...
    huffman_tables_identifiers = [(codebook_index, codebook_index) \
                                  for codebook_index in codebook_indexes]
...
def encode_progressive(channels: list[NDArray[np.uint8]],
    ...
    """
    Progressive encoding workflow
    """
    spectral_selectors = [(0, 0), (1, 5), (6, 26), (27, 63)]
    # Apply DCT and RL Encoding
    dct_encoded_channels = \
        get_dct_encoded_channels(channels, quantization_matrices, quantization_matrices_ids)
    print(spectral_selectors, dct_encoded_channels)
    data = bytearray()
    # NOT implemented yet
    return data
...
def pad_multichannel_image(multichannel_image_array: NDArray, custom_multiples: tuple[int, int] | None = None):
    """Function to Pad a MultiChannel Image"""
    _, _, depth = multichannel_image_array.shape
    padded_image = []
...
def jpeg_encode(img: ArrayLike, mode: int = JPEGMode.BASELINE,
    ...
    y_table = \
    np.clip(JPEGStandardTables.Y.astype(np.int64) * scaling_factor, 1, 255).astype(np.uint8)
    c_table = \
    np.clip(JPEGStandardTables.C.astype(np.int64) * scaling_factor, 1, 255).astype(np.uint8)
    ...
    if img_type == ImgType.GRAYSCALE:
        ...
        y_cr_cb_img = pad_multichannel_image(cv2.cvtColor(img,
    ...
    if thumbnail is not None:
        raise NotImplementedError("Thumbnails are not supported yet, leave it None")
    ...
    match mode:
        case JPEGMode.BASELINE:
            jpeg_data.extend(encode_baseline(channels,
                                             quantization_tables,
                                             channels_quantization_tables_ids))
        case JPEGMode.EXTENDED_SEQUENTIAL:
            raise NotImplementedError(f"{JPEGMode.EXTENDED_SEQUENTIAL} \
                                      mode is not implemented yet.")
        case JPEGMode.PROGRESSIVE:
            jpeg_data.extend(encode_progressive(channels,
                                                quantization_tables,
                                                channels_quantization_tables_ids))
        ...
        case JPEGMode.DIFFERENTIAL_SEQUENTIAL:
            raise NotImplementedError(f"{JPEGMode.DIFFERENTIAL_SEQUENTIAL} \
                                      mode is not implemented yet.")
        case JPEGMode.DIFFERENTIAL_PROGRESSIVE:
            raise NotImplementedError(f"{ JPEGMode.DIFFERENTIAL_PROGRESSIVE} \
                                      mode is not implemented yet.")
        case JPEGMode.DIFFERENTIAL_LOSSLESS:
            raise NotImplementedError(f"{JPEGMode.DIFFERENTIAL_LOSSLESS} \
                                      mode is not implemented yet.")
