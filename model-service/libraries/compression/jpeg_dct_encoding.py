"""
This module contains functions to encode JPEG DCT blocks.
"""
...
from libraries.compression.rle import get_diagonal_path
from libraries.compression.huffman import (index_to_bit_array,
                                           get_huffman_tree,
...
def escape_ff(scan_data: bytearray | bytes) -> bytearray:
    """
    Escapes 0xFF bytes in the scan data by inserting a zero byte after each 0xFF byte.
    This is necessary to ensure that the data can be correctly interpreted by JPEG decoders,
    as 0xFF is used as a marker in JPEG files.
    :param scan_data: The scan data to escape, as a bytearray or bytes.
    :return: A bytearray with 0xFF bytes escaped.
    """
...
def get_dct_transformed_block_elements(channel: NDArray,
                                       chunk_indexes: tuple[int, int],
                                       quantization_matrix: NDArray[np.float64]) \
                                        -> NDArray[np.int64]:
    """
    Takes a channel and a pair of chunk indexes, extracts the 8x8 chunk,
    applies DCT transformation, quantization, and returns the 64 elements
    in the zigzag order.
    :param channel: The channel to process, a 2D numpy array.
    :param chunk_indexes: A tuple of two integers representing the chunk indexes (y, x).
    :param quantization_matrix: The quantization matrix to use, a 2D numpy array.
    :return: A 1D numpy array of 64 elements in zigzag order.
    """
    ...
    chunk_transformed = np.array(dctn(chunk, type=2, norm='ortho'))
...
def get_channels_blocks_paths(channels: list[NDArray[np.uint8]]) -> list[NDArray[np.uint16]]:
    """
    Takes a list of channels and returns a list of paths for each channel,
    where each path contains the indexes of the 8x8 blocks in the channel.
    The first channel is assumed to have widths and heights multiples of following channels.
    The remaining channels are assumed to have the same shape.
    :param channels: A list of channels, each a 2D numpy array.
    :return: A list of numpy arrays, each containing the paths for the corresponding channel.
    """
...
class DCTEncodedBlock:
    """
    This class represents a DCT encoded block of 8x8 pixels.
    It contains the DC coefficient and the AC coefficients encoded in a way
    suitable for JPEG compression.
    The DC coefficient is encoded as a category and an index, while the AC coefficients
    are encoded as categories with their respective indexes.
    """
    ...
    def __init__(self, block_elements: NDArray[np.int64], last_dc: np.int64):
        ...
        dc_coefficient_category, dc_coefficient_index = \
            DCTEncodedBlock.CATEGORY_AND_INDEX[dc_coefficient]
        self.dc_class = dc_coefficient_category
        self.dc_coefficient_encoded_index = \
            index_to_bit_array(dc_coefficient_index, dc_coefficient_category)
        self.ac_classes, self.ac_coefficient_encoded_indexes = \
            self._get_ac_encoded_els(block_elements[1:])
    @staticmethod
    ...
    def get_encoded_block(self, dc_codebook: defaultdict, ac_codebook: defaultdict) -> bitarray:
        """Gets the encoded block"""
        assert len(self.ac_classes) == len(self.ac_coefficient_encoded_indexes), \
            "AC coefficients badly encoded"
        ...
        for ac_class, ac_coefficient_encoded_index in \
        zip(self.ac_classes, self.ac_coefficient_encoded_indexes):
...
def get_dct_encoded_channels(channels: list[NDArray[np.uint8]],
                         ...
                         quantization_matrices_ids: list[int]) -> list[NDArray]:
    """
    Obtains the DCT encoded channels
    """
    channels_blocks_paths = get_channels_blocks_paths(channels)
    n_mcus = channels_blocks_paths[0].shape[0]
    dct_encoded_channels = \
        [np.zeros(path.shape[:-1], dtype=DCTEncodedBlock) for path in channels_blocks_paths]
    for channel_index, channel in enumerate(channels):
        quantization_matrix = \
            quantization_matrices[quantization_matrices_ids[channel_index]].astype(np.float64)
        last_dc = np.int64(0)
        for mcu_index in range(n_mcus):
            for block_index, block_indexes in \
            enumerate(channels_blocks_paths[channel_index][mcu_index]):
...
def get_channel_codebooks(channel_blocks: NDArray) -> tuple[defaultdict, defaultdict]:
    """
    Obtains the channel codeblocks
    """
...
def encode_channel_blocks(channel_blocks: NDArray,
                          dc_codebook: defaultdict,
                          ac_codebook: defaultdict) -> NDArray:
    """
    Obtains the channel blocks
    """
    ...
    for mcu_index, mcu_block in enumerate(channel_blocks):
        for block_index, block in enumerate(mcu_block):
            encoded_channel_mcus[mcu_index, block_index] = \
                block.get_encoded_block(dc_codebook, ac_codebook)
    return encoded_channel_mcus


def get_encoded_scan(bit_encoded_channels_mcus: list[NDArray]) -> bytearray:
    """
    Obtains the encoded scan
    """
    scan = bitarray()
