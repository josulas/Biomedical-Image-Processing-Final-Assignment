"""
This module provides functionality to compute and plot the histogram of a grayscale image.
"""
...
class ImgHist:
    ...
    def __str__(self):
        return f"ImgHist(n_bins={self.n_bins}, \
            hist={self.hist}, bin_intervals={self.bin_intervals})"
    def __repr__(self):
        return f"ImgHist(n_bins={self.n_bins}, \
            hist={self.hist}, bin_intervals={self.bin_intervals})"
    def __eq__(self, other):
        return self.n_bins == other.n_bins and \
            np.array_equal(self.hist, other.hist) and \
            np.array_equal(self.bin_intervals, other.bin_intervals)
    def set_hist(self, hist, bin_intervals):
        """
        Set the histogram and bin intervals of the image
        @param hist: histogram of the image, a 1D numpy array with integer values
        @param bin_intervals: bin intervals of the histogram, 
        a 2D numpy array with shape (n_bins, 2)
        """
        self.hist = hist
        self.bin_intervals = bin_intervals
    def get_hist(self):
        """
        Get the histogram and bin intervals of the image
        @return: a tuple containing the histogram and the bin intervals
        """
        return self.hist, self.bin_intervals


def img2hist(img: ArrayLike,
             n_bins: int | None = None,
             v_min: int | None = None,
             v_max: int | None = None) -> ImgHist:
    """
    ...
    if v_max - v_min < n_bins - 1:
        raise ValueError("Number of bins must be lower or equal than " \
        "the range given by v_max and v_min")
...
def plot_hist(hist: ImgHist, **kwargs):
    ...
    if 'color' not in kwargs:
        kwargs['color'] = \
            kwargs['ax'].get_lines()[0].get_color() if kwargs['ax'].get_lines() else 'blue'
    ...
    if legend is not None:
        ax.bar(bin_intervals[:, 0],
               hist_values,
               width=bin_intervals[:, 1] - bin_intervals[:, 0],
               color=color,
               alpha=alpha,
           label=legend)
        ax.legend()
