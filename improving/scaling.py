...
def equalize_hist(img: ArrayLike) -> NDArray[np.uint8]:
    ...
    if img.ndim != 2 or not issubclass(img.dtype.type, np.uint8):
        ValueError('Image must be a 2D numpy array with 8-bit integer values')
...
def equalize_local_hist(img: ArrayLike, mask_size: int = 3, mode: str = 'same') -> NDArray[np.uint8]:
    """
    ...
    if not isinstance(mask_size, int) or mask_size < 3 or mask_size % 2 == 0:
        ValueError(f'Mask size must be an odd integer greater or equal than 3, got {mask_size} instead')
...
def intensity_scale(img: ArrayLike, f1: Real, f2: Real, f_max: numpy_real) -> np.float64:
    """
    ...
    if not isinstance(img, np.ndarray):
        img = np.array(img)
    if not img.ndim != 2 or not img.dtype.type == np.real:
        ValueError('Image must be a 2D numpy array with real values')
    if not f1 < f2:
